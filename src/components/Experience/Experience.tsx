...
export function Experience() {
    ...
    return (
        <LazyMotion features={domAnimation}>
            <section
                id="experience"
                className="flex h-auto min-h-screen flex-col overflow-hidden px-6 pb-6 xl:container md:px-12 md:pb-12 lg:px-28 xl:mx-auto"
            >
                <MotionDivReveal
                    ...
                    {prof_experience.map((item, index) => (
                        <div key={index} className="flex-col">
                            <MotionHeading3Reveal className=" dark:text-pale-blue">
                                {item.title} |{" "}
                                <a href={item.link} className="dark:text-orange/90 text-[#F15A59]">
                                    {item.company}
                                </a>
